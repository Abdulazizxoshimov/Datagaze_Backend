...
func (JWTHandler *JWTHandler) GenerateAuthJWT() (access, refresh string, err error) {
	...
	if err != nil {
		JWTHandler.Log.Error("error while generating access token with this error: %s", zap.Error(err))
		return
	}
	...
	if err != nil {
		JWTHandler.Log.Error("error while generating refresh token", zap.Error(err))
		return
	}
